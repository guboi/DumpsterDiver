...
def analyze_file(_file, result, settings):
    try:
        ...
        for word in get_all_strings_from_file(_file):
            ...
            if settings.skip_entropy is False:
        ...
        if settings.advance: 
            data = []
            if additional_checks.final(_file, data):
                result.put(data[0])
        ...
        if settings.secret:
            ...
            try:
                ...
                logger.error("while trying to open " +
                             str(_file) + ". Details:\n" + str(e))
    except Exception as e:
        import traceback
...
def get_all_strings_from_file(_file):
    with open(_file, 'r', encoding='utf-8', errors='replace') as open_file:
    ...
    try:
        ...
        for root, subfolder, files in os.walk(settings.local_path):
            for filename in files:
                ...
                if extension in excluded_files:
                    continue
                ...
                if extension in ARCHIVE_TYPES:
                    ...
                    try:
                        with open(_file, 'rb', encoding='utf-8') as f:
...
def git_object_reader(_file):
    try:
        git_object = open(_file, 'rb', encoding='utf-8').read()
...
def save_output(result, settings):
    try:
        ...
        logger.error("while trying to write to " +
                     str(settings.outfile) + " file. Details:\n" + str(e))
...
def bad_expression_verifier(_file, bad_expressions):
    try:
        with open(_file, 'rb', 0, encoding='utf-8') as f, \
        ...
        logger.error("while trying to open " + str(_file) +
                     " file. Details:\n" + str(e))
